...
public:
    ...
    FixedPoint(long long v) {
        if (v == 0) { mant = BigInt(0); sign = 0; scale = 0; }
        else { mant = BigInt(v < 0 ? -v : v); sign = v < 0 ? -1 : 1; scale = 0; }
    }
    ...
    static BigInt pow10BigInt(int k) {
        if (k < 0) throw runtime_error("pow10BigInt negative");
        if ((int)pow10_cache.size() > k) return pow10_cache[k];
        if (pow10_cache.empty()) pow10_cache.push_back(BigInt(1));
    ...
    void normalize() {
        ...
        if (mant.isZero()) sign = 0;
    }
 
    void roundKeep(int keep) {
        if (keep < 0) throw runtime_error("roundKeep negative");
        if (mant.isZero()) { sign = 0; scale = 0; return; }
        ...
        if (BigInt::cmp(twoR, p10) >= 0) q = q + BigInt(1);
        mant = q;
        scale = keep;
        normalize();
    }
    ...
    string toString(int outScale = -1) const {
        ...
        if (scale == 0) {
            ...
            if (outScale > 0) out += "." + string(outScale, '0');
            return out;
        }
        if ((int)ms.size() <= scale) ms = string(scale + 1 - (int)ms.size(), '0') + ms;
        string intpart = ms.substr(0, ms.size() - scale);
        string frac = ms.substr(ms.size() - scale);
        if (outScale >= 0) {
            if ((int)frac.size() < outScale) frac += string(outScale - frac.size(), '0');
            else if ((int)frac.size() > outScale) frac = frac.substr(0, outScale);
        } else {
            int t = (int)frac.size();
            while (t > 0 && frac[t - 1] == '0') --t;
            frac.resize(t);
        }
        string out = (sign < 0 ? "-" : "") + (intpart.empty() ? string("0") : intpart);
        if (!frac.empty()) out += "." + frac;
        else if (outScale > 0) out += "." + string(outScale, '0');
        return out;
    }
    ...
    friend FixedPoint operator+(const FixedPoint &A, const FixedPoint &B) {
        ...
        FixedPoint a = A, b = B;
        if (a.scale < b.scale) { int diff = b.scale - a.scale; a.mant = a.mant * pow10BigInt(diff); a.scale = b.scale; }
        else if (b.scale < a.scale) { int diff = a.scale - b.scale; b.mant = b.mant * pow10BigInt(diff); b.scale = a.scale; }
        FixedPoint res;
        if (a.sign == b.sign) { res.mant = a.mant + b.mant; res.sign = a.sign; res.scale = a.scale; }
        else {
            int c = BigInt::cmp(a.mant, b.mant);
            if (c == 0) return FixedPoint();
            else if (c > 0) { res.mant = a.mant - b.mant; res.sign = a.sign; res.scale = a.scale; }
            else { res.mant = b.mant - a.mant; res.sign = b.sign; res.scale = a.scale; }
        }
    ...
    friend FixedPoint operator-(const FixedPoint &A, const FixedPoint &B) { FixedPoint nb = B; nb.sign = -nb.sign; return A + nb; }
 
    friend FixedPoint operator*(const FixedPoint &A, const FixedPoint &B) {
        if (A.sign == 0 || B.sign == 0) return FixedPoint();
        FixedPoint res; res.mant = A.mant * B.mant; res.sign = A.sign * B.sign;
        long long ssum = (long long)A.scale + (long long)B.scale;
        if (ssum > INT_MAX) throw overflow_error("scale overflow");
        res.scale = (int)ssum;
        res.roundKeep(max(0, DEFAULT_PRECISION + 5));
        res.normalize();
        return res;
    ...
    friend FixedPoint operator/(const FixedPoint &A, const FixedPoint &B) {
        if (B.sign == 0) throw runtime_error("division by zero");
        if (A.sign == 0) return FixedPoint();
    ...
    friend bool operator==(const FixedPoint &A, const FixedPoint &B) {
        ...
        FixedPoint a = A, b = B;
        if (a.scale < b.scale) { a.mant = a.mant * pow10BigInt(b.scale - a.scale); a.scale = b.scale; }
        else if (b.scale < a.scale) { b.mant = b.mant * pow10BigInt(a.scale - b.scale); b.scale = a.scale; }
    ...
    friend bool operator<(const FixedPoint &A, const FixedPoint &B) {
        ...
        if (A.sign == 0) return B.sign > 0;
        FixedPoint a = A, b = B;
        if (a.scale < b.scale) { a.mant = a.mant * pow10BigInt(b.scale - a.scale); a.scale = b.scale; }
        else if (b.scale < a.scale) { b.mant = b.mant * pow10BigInt(a.scale - b.scale); b.scale = a.scale; }
        if (a.sign > 0) return a.mant < b.mant;
        else return a.mant > b.mant;
    }
    ...
    friend ostream& operator<<(ostream &os, const FixedPoint &x) { os << x.toString(); return os; }
};
