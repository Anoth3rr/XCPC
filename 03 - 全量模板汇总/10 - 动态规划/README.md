...
|05A / 05B / 05C|分治 DP / Knuth / 单调斜率 CHT|决策单调 / 四边形不等式 / 线性转移且斜率单调。|
|06A / 06B|LCS（常规 / Bitset）|恢复方案 / 只求字节串长度并卡常。|
|07|Slope Trick|维护凸分段线性函数，支持绝对值、前后缀最小化和滑动窗口平移。|
...
- 05C 与数据结构的 Li Chao 同为直线取最值：斜率递减、查询点递增时 05C 的 `askMono` 是均摊 `O(1)`；斜率或查询无序、需要线段直线时改用 Li Chao `02H`。WQS 二分见杂类 `05`，SOS DP 见数学 `08H`，换根 DP 见树上问题 `08`。
- `07` 只维护凸函数。`addAMX(a)` 与 `addXMA(a)` 分别加入 `(a-x)_+` 和 `(x-a)_+`；`shift(a,b)` 是滑动窗口最小化，不是简单把两个堆都加同一个数。调用前先检查转移是否真的保持凸性。
