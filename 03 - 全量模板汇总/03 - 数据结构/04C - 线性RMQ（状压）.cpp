...
template <class T, class Cmp = less<T>> struct LinearRMQ {
    ...
    LinearRMQ() = default;
    explicit LinearRMQ(const vector<T> &a, Cmp cmp = Cmp()) : cmp(move(cmp)) {
        build(a);
    }
    ...
    void build(const vector<T> &v) {
        ...
        st[0] = move(b);
        for (int k = 1; k <= lg; ++k)
            for (int i = 0; i + (1LL << k) <= nb; ++i) st[k][i] = best(st[k - 1][i], st[k - 1][i + (1LL << (k - 1))]);
    }
