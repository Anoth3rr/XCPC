class Decimal {
  public:
    ...
    friend std::ostream &operator<<(std::ostream &os, const Decimal &x) {
        ...
        while (trim > 0 && fracpart[trim - 1] == '0')
            --trim;
    ...
    friend int cmp(const Decimal &a, const Decimal &b) {
        ...
        if (a.scale < b.scale) {
            ...
            ta.mant = BigInt::mulInt(ta.mant, pow10Int(diff));
            ta.scale = b.scale;
        } else {
            int diff = (int)(a.scale - b.scale);
            tb.mant = BigInt::mulInt(tb.mant, pow10Int(diff));
            tb.scale = a.scale;
        }
    ...
    Decimal sqrt() const {
        ...
        for (int iter = 0; iter < 60; ++iter) {
            ...
            Decimal diff = nx - x;
            diff.normalize();
            if (diff.isZero() || diff.absLessThanPow10(-prec - 2)) {
                x = nx;
                break;
            }
            x = nx;
  ...
  private:
    ...
    static BigInt pow10BigInt(int k) {
        ...
        for (int i = (int)cache.size(); i <= k; ++i)
            cache.push_back(BigInt::mulInt(cache.back(), 10));
        return cache[k];
    }
