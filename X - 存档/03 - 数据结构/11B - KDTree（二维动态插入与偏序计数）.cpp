template <class T = int> struct KDTree {
    ...
    bool bad(int p) const {
        return max(sz(t[p].l), sz(t[p].r)) * 4 > sz(p) * 3;
    }
    ...
    void ins(int &p, int q) {
        ...
        if ((!t[p].d && pair(t[q].x, t[q].y) < pair(t[p].x, t[p].y)) || (t[p].d && pair(t[q].y, t[q].x) < pair(t[p].y, t[p].x)))
            ins(t[p].l, q);
        else
            ins(t[p].r, q);
        pull(p);
