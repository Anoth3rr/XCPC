...
template <class T = int, class D = i128> struct KDTree {
    ...
    KDTree() = default;
    explicit KDTree(const vector<Point> &p) {
        build(p);
    }
    ...
    static D dis(const Point &a, const Point &b) {
    ...
    int make(int l, int r, int d) {
        ...
        int u = tr.size();
        tr.push_back({a[m].first, a[m].first.x, a[m].first.x, a[m].first.y, a[m].first.y, -1, -1, a[m].second, 1});
        tr[u].l = make(l, m, d ^ 1);
