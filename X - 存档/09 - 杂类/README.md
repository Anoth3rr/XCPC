...
这里存放 XCPC / NOI 中会用到、但不适合归入基础算法、树、数据结构、图论、网络流、数学、字符串或高精度的代码。
...
|15|麻将胡牌判定（标准、七对子、国士）|34 种牌编码下的十四张手牌判定；不含番数和地方规则。|

另见 [杂类知识与结论](杂类知识与结论.md)，包含莫队与离线算法的适用条件、笛卡尔树性质、WQS/决策单调性、精确覆盖、随机化及语言差异；赛时检查清单见 [错题本](错题本.md)。
...
- 麻将模板只判三种常见和牌结构：标准型、七对子、国士无双；副露、番数、听牌和地方规则不在代码语义内，题面若有特殊规则需另写状态。
