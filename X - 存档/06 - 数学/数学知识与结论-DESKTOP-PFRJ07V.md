...
- 求最小乘法阶（最小正整数满足 $a^x\equiv 1\pmod m$ ）使用 02K。
- 前提：$m>1$ 且 $\gcd(a,m)=1$。先取群阶 $k=\varphi(m)$（若已知更紧的 $\lambda(m)$ 或其他群阶，可直接传入），再对 $k$ 的每个不同质因子 $p$ 检查 $a^{k/p}$；若等于 1 就令 $k\leftarrow k/p$，最终的 $k$ 即为 $\operatorname{ord}_m(a)$。

## 1.4 常用数量级与模板自检数据（WIDA 对照）
...
不超过 $10^9$ 的相邻素数最大间隔为 282（WIDA 统计值）。

设 $T(N)=\max_{n\le N}\tau(n)$。WIDA 原表和按 $\tau(n)$ 穷举复核值同时列出：

| $N$ 量级 | $10$ | $10^2$ | $10^3$ | $10^4$ | $10^5$ | $10^6$ | $10^7$ |
|---|---:|---:|---:|---:|---:|---:|---:|
| WIDA 原表 $T(N)$ | 4 | 25 | 32 | 64 | 128 | 240 | 448 |
| 复核 $T(N)$ | 4 | 12 | 32 | 64 | 128 | 240 | 448 |
| 代表数字 | 6,8,10 | 60,72,84,90,96 | 840 | 7560,9240 | 83160,98280 | 720720,831600,942480,982800,997920 | 8648640 |

`10^2` 的 25 是 WIDA 原表笔误；不超过 100 的最大约数个数应为 12。

WIDA 组合数自检值：

$$
\binom{132}{66}=377389666165540953244592352291892721700,
$$
...
Catalan 前十项（$n=0\ldots9$）：`1 1 2 5 14 42 132 429 1430 4862`。

常用短回归值：

| 模板/运算 | 输入 | 期望值 |
|---|---|---|
|gcd，$\varphi$，$\mu$|$\gcd(48,18)$；$\varphi(36)$；$\mu(30),\mu(12)$|6；12；-1，0|
|约数与最小乘法阶|$\tau(720720),\sigma(12)$；$\operatorname{ord}_7(3),\operatorname{ord}_8(3),\operatorname{ord}_{10}(3),\operatorname{ord}_{12}(5)$|240，28；6，2，4，2|
|模逆、CRT、BSGS|$3^{-1}\bmod998244353$；$x\equiv2\pmod3,x\equiv3\pmod5$；$2^x\equiv8\pmod{13}$|332748118；$x\equiv8\pmod{15}$；3|
|Stirling / Bell / 分拆|$S(5,k)$：0，1，15，25，10，1；$B_0\ldots B_{10}$|对应序列见主知识稿|

Bernoulli 采用 $B_1=-1/2$ 时，$B_0\ldots B_6=1,-1/2,1/6,0,-1/30,0,1/42$；NTT 基本卷积 `[1,2,3]*[4,5]` 应为 `[4,13,22,15]`。
